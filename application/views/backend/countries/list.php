		...
		<?php foreach($countries->result() as $country): ?>
			...
			<tr>
				...
				<?php if ( $this->ps_auth->has_access( EDIT )): ?>
					...
					<td>
						<a href='<?php echo $module_site_url .'/edit/'. $country->id; ?>'>
				...
				<?php if ( $this->ps_auth->has_access( DEL )): ?>
					...
					<td>
						<a herf='#' class='btn-delete' data-toggle="modal" data-target="#myModal" id="<?php echo "$country->id";?>">
				...
				<?php if ( $this->ps_auth->has_access( PUBLISH )): ?>
					...
					<td>
						<?php if ( @$country->status == 1): ?>
							<button class="btn btn-sm btn-success unpublish" id='<?php echo $country->id;?>'>
							<?php echo get_msg('btn_yes'); ?></button>
						<?php else:?>
							<button class="btn btn-sm btn-danger publish" id='<?php echo $country->id;?>'>
