...
class Countries extends BE_Controller {
	...
	function search() {
		...
		// breadcrumb urls
		$this->data['action_title'] = get_msg( 'country_search' );
	...
	function add() {
		...
		// breadcrumb urls
		$this->data['action_title'] = get_msg( 'country_add' );
	...
	function save( $id = false ) {
		...
		if ( ! $this->check_trans()) {
			...
			// if user id is not false, show success_add message
				...
				$this->set_flash_msg( 'success', get_msg( 'success_country_edit' ));
			} else {
			// if user id is false, show success_edit message

				$this->set_flash_msg( 'success', get_msg( 'success_country_add' ));
			}
		}
	...
	*/
	function edit( $id ) 
	{
		...
		// breadcrumb urls
		$this->data['action_title'] = get_msg( 'country_edit' );
  	...
  	*/
  	function delete( $id ) {
	    ...
	    */
	     ...
	     $this->set_flash_msg( 'success', get_msg( 'success_country_delete' ));
	    }
	...
	function is_valid_name( $name, $id = 0, $shop_id = 0 )
	{
		...
		if( $id != "") {
			if ( strtolower( $this->Country->get_one( $id )->name ) == strtolower( $name )) {
			// if the name is existing name for that user id,
			return true;
			}else if ( $this->Country->exists( ($conds ))) {
			// if the name is existed in the system,
			$this->form_validation->set_message('is_valid_name', get_msg( 'err_dup_name' ));
			return false;
			}
		} else {
			if ( $this->Country->exists( ($conds ))) {
				// if the name is existed in the system,
				$this->form_validation->set_message('is_valid_name', get_msg( 'err_dup_name' ));
				return false;
			}
	 ...
	 * Publish the record
	 *
	 * @param      integer  $Country_id  The Country identifier
	 */
	function ajx_publish( $country_id = 0 )
	{
		// check access
		$this->check_access( PUBLISH );
		
		// prepare data
		$country_data = array( 'status'=> 1 );
			
		// save data
		if ( $this->Country->save( $country_data, $country_id )) {
			echo true;
		} else {
			echo false;
		}
	 ...
	 * @param      integer  $Country_id  The Country identifier
	 */
	function ajx_unpublish( $country_id = 0 )
	{
		// check access
		$this->check_access( PUBLISH );
		
		// prepare data
		$country_data = array( 'status'=> 0 );
